...
void	AForm::execute(Bureaucrat const &executor) const {
	...
	if (this->_isSigned == false) {
		std::cout << executor.getName() << ": " << this->_name << " ";
		throw AForm::FormNotSignedException();
	}
	else if (executor.getGrade() > this->_gradeToExecute) {
		std::cout << executor.getName() << " about " << this->_name << ": ";
		std::cout << "my [" << executor.getGrade() << "] "; 
		throw AForm::GradeTooLowException();
	}
...
const char *AForm::GradeTooHighException::what() const throw() {
	...
	return MAGENTA "Grade is too high, cannot execute\n" RESET;
}

const char *AForm::GradeTooLowException::what() const throw() {

	return MAGENTA "Grade is too low, cannot execute\n" RESET;
}

const char *AForm::FormNotSignedException::what() const throw() {

	return MAGENTA "\"form is Not signed, cannot execute\"\n" RESET;
}
...
/*
** `this->executeAction()` will be overridden by derived classes.
** `this` is a pointer to the current object.
**
** For example, in main function we used `shrubbery.executeAction(bob)`,
** `shrubbbery` is an object of the `ShrubberyCreationForm` class.
** So, `this->executeAction()` will call the `executeAction()` method
** of the `ShrubberyCreationForm` class.
*/
